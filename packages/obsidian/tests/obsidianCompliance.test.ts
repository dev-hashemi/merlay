...
const OBSIDIAN_PKG = join(REPO, 'packages', 'obsidian');
...
test('Obsidian compliance: manifest has all required fields', () => {
  const manifest = JSON.parse(readFileSync(join(OBSIDIAN_PKG, 'manifest.json'), 'utf8')) as Record<
    string,
    unknown
  >;
  for (const field of [
...
test('Obsidian compliance: manifest version matches package.json and versions.json', () => {
  const manifest = JSON.parse(readFileSync(join(OBSIDIAN_PKG, 'manifest.json'), 'utf8')) as {
    version: string;
  };
  const pkg = JSON.parse(readFileSync(join(OBSIDIAN_PKG, 'package.json'), 'utf8')) as {
    version: string;
  };
  const versions = JSON.parse(readFileSync(join(OBSIDIAN_PKG, 'versions.json'), 'utf8')) as Record<
    string,
    string
  >;
  assert.strictEqual(
...
test('Core purity: no host DOM extensions in portable core', () => {
  ...
  assertNoMatch(
    coreFiles(),
    /(^|[^.\w])createSvg\s*\(/,
    'use createSvgElement() from src/platform/dom instead of the host global'
  );
});
